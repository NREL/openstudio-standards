...
class VentDef621(DBOperation):
    ...
    def validate_record_datatype(self, record):
        if record.get("ventilation_per_person"):
            assert is_float(
                record.get("ventilation_per_person")
            ), f"lighting_per_area requires to be numeric data type, instead got {record['ventilation_per_person']}"
        if record.get("ventilation_per_area"):
            assert is_float(
                record.get("ventilation_per_area")
            ), f"ventilation_per_area requires to be numeric data type, instead got {record['ventilation_per_area']}"
        if record.get("occupancy_per_area"):
            assert is_float(
                record.get("occupancy_per_area")
            ), f"occupancy_per_area requires to be numeric data type, instead got {record['occupancy_per_area']}"
