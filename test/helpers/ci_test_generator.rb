...
module CITestGenerator
  ...
  def self.generate(copy_ci = false)
    ...
    write_file_path_to_ci_tests_txt(copy_ci)
  end
...
class Test_#{building_type}_#{template}_#{fuel_type} < NECBRegressionHelper
  ...
  def self.write_file_path_to_ci_tests_txt(copy_ci)
    circleci_tests_txt_path = File.absolute_path(File.join(__FILE__, "..", "..", "circleci_tests.txt"))

    if copy_ci
      orig_ci_path = circleci_tests_txt_path
      new_ci_path = File.absolute_path(File.join(__FILE__, "..", "..", "local_circleci_tests.txt"))
      FileUtils.copy(orig_ci_path, new_ci_path)
      circleci_tests_txt_path = new_ci_path
    end
