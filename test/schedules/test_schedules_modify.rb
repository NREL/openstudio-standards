...
require_relative '../../lib/openstudio-standards/schedules/modify'
...
class TestSchedulesModify < Minitest::Test
  ...
  include OpenstudioStandards::Schedules::Modify
  ...
  def test_schedule_ruleset_simple_value_adjust
    ...
    schedule_ruleset_simple_value_adjust(schedule, 0.5)
    assert(schedule.to_ScheduleRuleset.is_initialized)
    schedule_max = schedule_ruleset_min_max(schedule)['max']
    assert(schedule_max == 1.5)

    schedule_ruleset_simple_value_adjust(schedule, 0.5, 'Sum')
    assert(schedule.to_ScheduleRuleset.is_initialized)
    schedule_max = schedule_ruleset_min_max(schedule)['max']
  ...
  def test_schedule_ruleset_conditional_adjust_value
    ...
    fail_value = 2.0
    floor_value = 0.1
    schedule_ruleset_conditional_adjust_value(schedule, test_value, pass_value, fail_value, floor_value)
    schedule_max = schedule_ruleset_min_max(schedule)['max']
  ...
  def test_schedule_ruleset_time_conditional_adjust_value
    ...
    inside_value = 2.0
    outside_value = 1.0
    schedule_ruleset_time_conditional_adjust_value(schedule, hhmm_before, hhmm_after, inside_value, outside_value)
  end
  ...
  def test_schedule_ruleset_adjust_hours_of_operation
    ...
    schedule_ruleset_adjust_hours_of_operation(schedule, basic_shift)
    model.save('test_schedule_ruleset_adjust_hours_of_operation.osm')
  end
end