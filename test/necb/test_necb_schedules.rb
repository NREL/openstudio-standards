...
#        puts spacetype_occ_sched
#        if occ_sched_ruleset == spacetype_occ_sched
#          sched_rules_2 = occ_sched_ruleset.scheduleRules
#          sched_rules = spacetype_occ_sched.scheduleRule.get
#          puts "Why doesn't this work!??!!"
#        end
#        spcaetype_occ_scheddays = st.defaultScheduleSet.get.numberofPeopleSchedule.get.scheduleRulesets
#        spacetype_occ_scheddays = spacetype_occ_sched.scheduleRules.get

=begin
        scheds = @model.getSchedules
        @model.getSchedules.each do |sched|
          schedname = sched.name.get
          puts "Schedule name = #{schedname}, looking for #{spacetype_occ_sched}"
          if sched.name.get.to_s == spacetype_occ_sched.to_s
            schedday = sched.scheduleTypeLimits.get
            schedday.each do |sched_limits|
              sched_limits_name = sched_limits.name
            end
            puts "Worked?"
          end
        end
        ...
        #        spacetype_lighting_sched = defaultScheduleSet.lightingSchedule.get.name
=end
