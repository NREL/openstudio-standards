...
class RunAllTests< Minitest::Test  
   ...
   def test_all()
    ...
    File.open(File.join(File.dirname(__FILE__), 'helpers', 'ci_test_helper', 'timings.json'), 'w') { |file| file.puts(JSON.pretty_generate(timings_json.sort_by{ |key, value| key }))}
