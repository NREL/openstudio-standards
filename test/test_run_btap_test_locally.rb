...
class RunBTAPTests < Minitest::Test
  def test_all()
    full_file_list = nil
    if File.exist?(TestListFile)
      ...
      full_file_list.select! {|item| item.include?('rb') && item.include?('btap') && File.exist?(File.absolute_path(File.join(File.dirname(__FILE__),"#{item.strip}")))}
