...
class HVACEfficienciesTest < MiniTest::Test
  ...
  def test_heatpump_efficiency
    ...
    if min_caps.size == 1
      ...
      test_caps << 0.5 * (OpenStudio.convert(min_caps[0].to_f, 'Btu/hr', 'W').to_f + OpenStudio.convert(min_caps[1].to_f, 'Btu/h', 'W').to_f)
      if min_caps.size == 2
        test_caps << (OpenStudio.convert(min_caps[1].to_f, 'Btu/hr', 'W').to_f + 10000.0)
      else
        test_caps << 0.5 * (OpenStudio.convert(min_caps[1].to_f, 'Btu/hr', 'W').to_f + OpenStudio.convert(min_caps[2].to_f, 'Btu/hr', 'W').to_f)
        test_caps << (min_caps[2].to_f + 10000.0)
      end
    end
