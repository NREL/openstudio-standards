...
#      
#    template = 'NECB 2011'
#    #Go Through each space type.
#    @model.find_objects($os_standards["space_types"], { "template" => 'NECB 2011'}).each do |space_type_properties|
#      # Create a space type
#      st = OpenStudio::Model::SpaceType.new(@model)
#      st.setStandardsBuildingType(space_type_properties['building_type'])
#      st.setStandardsSpaceType(space_type_properties['space_type'])
...
#      st.set_rendering_color(template)
#      @model.add_loads(template)
#      #Assign space type to a spacetype and thermal zone. 
#      @model.getSpaces.each do |space| 
#        space.setSpaceType(st)
#        BTAP::Geometry::Zones::create_thermal_zone(@model, [space])
#      end
#      #set constructions
#      @model.clear_and_set_example_constructions()
#      puts "DONE!!!"
...
#      assert(@model.runSizingRun("#{Dir.pwd}/SpaceSizingRun"),'SizingRun Failed')
#  
#        
...
#      
#    #Run System picker
#    #Output spacetype, system, number of floor, cooling capacity, heating capacity and if it is refrigerated. 
#    #loop
#    #Put results to casv file. 
#      
