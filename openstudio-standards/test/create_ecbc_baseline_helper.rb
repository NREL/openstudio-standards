...
def load_test_model(model_name)
  ...
  path = OpenStudio::Path.new("#{File.dirname(__FILE__)}/test_models/ecbc_baseline/#{model_name}.osm")
  model = translator.loadModel(path)
  assert(model.is_initialized, "Could not load test model '#{model_name}.osm' from test_models/ecbc_baseline.  Check name for typos.")
  model = model.get
...
def create_ecbc_baseline_model(model_name, standard, climate_zone, building_type, custom = nil, debug = false, load_existing_model = true, operation_type = "Daytime")
  ...
  if model.weatherFile.empty?
    ...
    case climate_zone
    when 'ECBC Composite'
      epw_name = 'IND_Hyderabad.431280_ISHRAE.epw'
    when 'ECBC Hot and Dry'
      epw_name = 'IND_Ahmedabad.426470_ISHRAE.epw'
    when 'ECBC Warm and Humid'
      epw_name = 'IND_Pune.430630_ISHRAE.epw'
    when 'ECBC Moderate'
