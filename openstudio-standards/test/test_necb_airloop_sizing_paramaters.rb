...
class HVACEfficienciesTest < MiniTest::Test
  ...
  def test_airloop_sizing_rules
    ...
    assert_equal(true, result, "test_airloop_sizing_rules: Failure in Standards for #{name}")
    ...
    airloops.each do |iloop|
      ...
      thermal_zones.each do |izone|
        ...
        assert(heating_sizing_factor_set_correctly, "test_airloop_sizing_rules: Heating sizing factor does not match necb requirement #{name}")
        ...
        assert(cooling_sizing_factor_set_correctly, "test_airloop_sizing_rules: Cooling sizing factor does not match necb requirement #{name}")
        ...
        assert(heating_sizing_temp_set_correctly, "test_airloop_sizing_rules: Heating sizing supply temperature does not match necb requirement #{name}")
        diff = (heating_sizing_temp.to_f - necb_heating_sizing_temp).abs / necb_heating_sizing_temp
        cooling_sizing_temp_set_correctly = true
        if diff > tol then cooling_sizing_temp_set_correctly = false end
        assert(cooling_sizing_temp_set_correctly, "test_airloop_sizing_rules: Cooling sizing supply temperature does not match necb requirement #{name}")
      end
    end
  end
