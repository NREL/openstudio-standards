  ...
  class StatFile
    ...
    def parse(text)
      ...
      if match_data.nil?
        ...
        @lat = match_data[2].to_f + match_data[3].to_f / 60.0 +  match_data[4].to_f / 100.0 / 60.0
        if match_data[1] == 'S'
          @lat = -@lat
        end

        @lon = match_data[6].to_f + match_data[7].to_f / 60.0 + match_data[8].to_f / 100.0 / 60.0
        if match_data[4] == 'W'
          @lon = -@lon
        end
      ...
      cdd_10_regex = /-\s*(.*) annual \((standard|wthr file)\) cooling degree-days \(10C baseline\)/
      ...
      hdd_10_regex = /-\s*(.*) annual \((standard|wthr file)\) heating degree-days \(10C baseline\)/
