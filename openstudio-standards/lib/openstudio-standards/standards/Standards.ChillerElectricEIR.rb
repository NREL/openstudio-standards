...
class OpenStudio::Model::ChillerElectricEIR
  ...
  def standard_minimum_full_load_efficiency(template, standards)
    ...
    chlr_props = model.find_object(standards['chillers'], search_criteria, capacity_tons)
  ...
  def set_efficiency_and_curves(template, clg_tower_objs)
    ...
    # Get the chiller properties
    chlr_props = model.find_object(chillers, search_criteria, capacity_tons)
    unless chlr_props
