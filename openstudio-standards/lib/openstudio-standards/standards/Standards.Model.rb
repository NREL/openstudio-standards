...
class OpenStudio::Model::Model
  ...
  def add_construction(construction_name, construction_props = nil)
    ...
    if construction_props
      ...
      if data['skylight_framing']
        ...
        frame_data = find_object($os_standards['materials'], 'name' => framing_name)
        if frame_data
          ...
          frame = OpenStudio::Model::WindowPropertyFrameAndDivider.new(self)
          ...
          self.getSubSurfaces.each do |sub_surface|
  ...
  def add_curve(curve_name)
    ...
    # Find curve data
    data = find_object($os_standards['curves'], 'name' => curve_name)
    if data.nil?
    ...
    case data['form']
      when 'Linear'
        curve = OpenStudio::Model::CurveLinear.new(self)
