...
Dir.glob("#{refactor_standards_dir}/**/*.rb").each do |file_path|
  ...
  lines.each do |line|
    ...
    if line.strip[0] == '#' # Skip comment lines
      ...
      new_line = line.gsub('#{template}', '#{@@template}') # Use @@template in messages
      new_lines << new_line
      puts "#{line} => #{new_line}"
    elsif line.include?('template ==')
      new_line = line.gsub('template ==', '@@template ==') # Use @@template in if statements
      new_lines << new_line
      puts "#{line} => #{new_line}"  
    elsif line.include?("'template' => template")
      new_line = line.gsub("'template' => template", "'template' => @@template") # Use @@template in search criteria
      new_lines << new_line
      puts "#{line} => #{new_line}"
    elsif line.include?("'template' => @@template") # Skip already modified lines
      new_lines << line
      next
      ...
      new_lines << new_line
      puts "#{line} => #{new_line}"           
    elsif line.include?('template') # Replace calls with class variable @@template
      new_line = line.gsub('template', '@@template')
      new_line = new_line.gsub("'@@template'", "'template'")
      new_line = line.gsub('@@template', '@@template')
      new_lines << new_line
      puts "#{line} => #{new_line}"
    else
