    ...
    keys.sort( &block ).reduce( { } ) do | seed , key |
      ...
      if recursive && seed[ key ].is_a?( Hash )
        ...
        if key == 'space_types' # Don't have names
          seed[ key ] = seed[ key ].sort_by { | hsh | 
            [ hsh[ 'template' ] , hsh[ 'climate_zone_set' ] , hsh[ 'building_type' ] , hsh[ 'space_type' ] ] 
          }
        
        elsif key == 'schedules' # Names are not unique, sort by name then day types
          seed[ key ] = seed[ key ].sort_by { | hsh | 
            [ hsh['name'] , hsh[ 'start_date' ] , hsh[ 'day_types' ] ] 
          }
          ...
          seed[key] = seed[key].sort_by { | hsh | 
            [ hsh[ 'template' ] , hsh[ 'building_type' ] , hsh[ 'space_type' ] ] 
          } #, hsh['exterior_walls'], hsh['exterior_roofs'], hsh['exterior_floors']] }
          ...
          begin
            ...
            puts 'Error: Could not sort names.'
          end
