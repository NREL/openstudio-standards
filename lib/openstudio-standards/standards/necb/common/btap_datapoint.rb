...
class BTAPDatapoint
  def initialize(input_folder: nil,
    ...
    begin
      ...
      # If an annual simulation was not run, then the output.osm file and 
      # eplusout.sql files are needed to perform costing and carbon calculation. 
      else
        @standard = Standard.build(@options[:template])
        model = BTAP::FileIO.load_osm(File.join(input_folder, 'output.osm'))
        model.setSqlFile(OpenStudio::SqlFile.new(File.join(input_folder, 'eplusout.sql')))
        @qaqc = qaqc_build(model)
      end
