...
class NECB2011 < Standard
  ...
  def initialize
    ...
    # New BTAP::Defaults instance. Can be overwritten with 'set_defaults'.
    @defaults = BTAP::Defaults.new
  ...
  ##
  # Generates one of the 16 archetypes, which is subsequently customized based
  # on NECB-related parameters.
  #
  # @return [OpenStudio::Model::Model] a generated, NECB-customized model
  def _model_create_prototype_model
    model = load_building_type_from_library

    model_apply_standard(model: model)
  end
  ...
  ##
  # Sets primary heating fuel(s).
  #
  # @param [OpenStudio::Model::Model] a model
  #
  # @return [Boolean] true if successful
  def set_heating_fuels(model: nil)
    return false unless model.is_a?(OpenStudio::Model::Model)

    primary_heating_fuel = validate_primary_heating_fuel(model: model)
    @defaults.argh[:primary_heating_fuel] = primary_heating_fuel
    self.fuel_type_set = SystemFuels.new
    self.fuel_type_set.set_defaults(standards_data: @standards_data,
                                    primary_heating_fuel: primary_heating_fuel)
  ...
  ##
  # Customizes an OpenStudio model with NECB-related parameters (defaults.rb)
  #
  # @param [OpenStudio::Model::Model] a model
  #
  # @return [OpenStudio::Model::Model] an NECB-customized model (nil if invalid)
  def _model_apply_standard(model: nil)
    return nil unless model.is_a?(OpenStudio::Model::Model)

    apply_weather_data(model: model)
    set_heating_fuels(model: model)
