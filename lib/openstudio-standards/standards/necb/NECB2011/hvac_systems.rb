class NECB2011
  ...
  def boiler_hot_water_apply_efficiency_and_curves(boiler_hot_water)
    ...
    blr_props = model_find_object(table_name: 'boilers', search_criteria: search_criteria, capacity: capacity_btu_per_hr)
    unless blr_props
  ...
  def chiller_electric_eir_apply_efficiency_and_curves(chiller_electric_eir, clg_tower_objs)
    ...
    chlr_props = model_find_object(table_name: 'chillers', search_criteria: search_criteria, capacity: capacity_tons, date: Date.today)
    unless chlr_props
  ...
  def coil_cooling_dx_multi_speed_apply_efficiency_and_curves(coil_cooling_dx_multi_speed, sql_db_vars_map)
    ...
    ac_props = if heat_pump == true
                 model_find_object(table_name: 'heat_pumps', search_criteria: search_criteria, capacity: capacity_btu_per_hr, date: Date.today)
               else
                 model_find_object(table_name: 'unitary_acs', search_criteria: search_criteria, capacity: capacity_btu_per_hr, date: Date.today)
               end
  ...
  def fan_standard_minimum_motor_efficiency_and_size(fan, motor_bhp)
    ...
    if fan_small_fan?(fan)
      ...
      motor_properties = model_find_object(table_name: 'motors', search_criteria: search_criteria, capacity: motor_bhp)
    ...
    motor_properties = model_find_object(table_name: 'motors', search_criteria: search_criteria, capacity: nominal_hp + 0.01)
  ...
  def necb_spacetype_system_selection(model, heating_design_load = nil, cooling_design_load = nil)
    ...
    model.getSpaces.sort.each do |space|
      ...
      if space.spaceType.empty?
        ...
        space_type_property = model_find_object(table_name: 'space_types', search_criteria: {'template' => @template,
