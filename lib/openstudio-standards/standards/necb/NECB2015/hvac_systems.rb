    ...
    plant_loops.each do |plantloop|
      ...
      plantloop.supplyComponents.each do |supplycomp|
        case supplycomp.iddObjectType.valueName.to_s
          when 'OS_CentralHeatPumpSystem', 'OS_Coil_Heating_WaterToAirHeatPump_EquationFit','OS_Coil_Heating_WaterToAirHeatPump_VariableSpeedEquationFit','OS_Coil_Heating_WaterToAirHeatPump_VariableSpeedEquationFit_SpeedData','OS_HeatPump_WaterToWater_EquationFit_Cooling','OS_HeatPump_WaterToWater_EquationFit_Heating'
            max_powertoload = model_find_object(table_name: 'max_total_loop_pump_power', search_criteria: {'hydronic_system_type' => 'WSHP'})['total_normalized_pump_power_wperkw']
      ...
      unless max_powertoload > 0
        case plantloop.sizingPlant.loopType
          when 'Heating'
            max_powertoload = model_find_object(table_name: 'max_total_loop_pump_power', search_criteria: {'hydronic_system_type' => 'Heating'})['total_normalized_pump_power_wperkw']
          when 'Cooling'
            max_powertoload = model_find_object(table_name: 'max_total_loop_pump_power', search_criteria: {'hydronic_system_type' => 'Cooling'})['total_normalized_pump_power_wperkw']
          when 'Condenser'
            max_powertoload = model_find_object(table_name: 'max_total_loop_pump_power', search_criteria: { 'hydronic_system_type' => 'Heat_rejection'})['total_normalized_pump_power_wperkw']
        end
      end
