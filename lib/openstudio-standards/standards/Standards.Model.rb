...
class Standard
  ...
  # Create an hourly ScheduleRuleset based on 8760 input values
  #
  # @param model [OpenStudio::Model::Model] OpenStudio model object
  # @param values [Array<Double>] array of the 8760 values to use
  # @param name [String] the name of the schedule
  # @param sch_type_limit [String] the name of a schedule type limit
  #   options are Temperature, Humidity Ratio, Fractional, OnOff, and Activity
  # @return [OpenStudio::Model::ScheduleRuleset] schedule ruleset object
  def model_add_8760_schedule_ruleset(model, values, sch_name,
    ...
    # check to see if schedule exists with same name and constant value and return if true
    unless sch_name.nil?
      existing_sch = model.getScheduleRulesetByName(sch_name)
      if existing_sch.is_initialized
        OpenStudio.logFree(OpenStudio::Warn, 'openstudio.Model.Model', "Schedule #{sch_name} already exists.")
        existing_sch = existing_sch.get
        return existing_sch
      end
    end
    ...
    if values.length != 8760
      OpenStudio.logFree(OpenStudio::Error, 'openstudio.Model.Model', 'Input array does not equal to 8760 values.')
    end
    ...
    if !sch_type_limit.nil?
      sch_type_limits_obj = model_add_schedule_type_limits(model, standard_sch_type_limit: sch_type_limit)
      sch_ruleset.setScheduleTypeLimits(sch_type_limits_obj)
    end

    # Default - All hours constant
    unless default_value.nil?
      day_sch = sch_ruleset.defaultDaySchedule
      day_sch.setName("#{sch_name} Default")
      day_sch.addValue(OpenStudio::Time.new(0, 24, 0, 0), default_value)
    end

    # Winter Design Day - All hours constant
    unless winter_value.nil?
      day_sch = OpenStudio::Model::ScheduleDay.new(model)
      sch_ruleset.setWinterDesignDaySchedule(day_sch)
      day_sch = sch_ruleset.winterDesignDaySchedule
      day_sch.setName("#{sch_name} Winter Design Day")
      day_sch.addValue(OpenStudio::Time.new(0, 24, 0, 0), winter_value)
    end

    # Summer Design Day - All hours constant
    unless summer_value.nil?
      day_sch = OpenStudio::Model::ScheduleDay.new(model)
      sch_ruleset.setSummerDesignDaySchedule(day_sch)
      day_sch = sch_ruleset.summerDesignDaySchedule
      day_sch.setName("#{sch_name} Summer Design Day")
      day_sch.addValue(OpenStudio::Time.new(0, 24, 0, 0), summer_value)
    end

    # get model information
    model_run_period = model.getRunPeriod
    year = model.getYearDescription
    weekday_start = model.dayofWeekforStartDay
    first_date_of_year = year.makeDate(1)

    # loop through the array to make daily schedule ruleset
    day_of_year = first_date_of_year.dayOfYear
    values.each_slice(24) do |daily_array|
      current_date_of_year = year.makeDate(day_of_year)
      current_month_name = current_date_of_year.monthOfYear.valueName
      current_day = current_date_of_year.dayOfMonth
      current_weekday_name = current_date_of_year.dayOfWeek.valueName

      # create new day rule
      sch_rule = OpenStudio::Model::ScheduleRule.new(sch_ruleset)
      sch_rule.setName("#{current_month_name}-#{current_day} Rule")
      day_sch = sch_rule.daySchedule
      day_sch.setName("#{current_month_name}-#{current_day} Day Schedule")

      # loop through each hourly value
      daily_array.each_with_index do |val, hour|
        day_sch.addValue(OpenStudio::Time.new(0, hour + 1, 0, 0), val)
      end

      # Set the dates when the rule applies
      sch_rule.setStartDate(current_date_of_year)
      ...
      if current_date_of_year >= start_of_last_week
        ...
        sch_rule.setEndDate(current_date_of_year)
      end

      # Individual Days
      sch_rule.setApplyMonday(true) if current_weekday_name == 'Monday'
      sch_rule.setApplyTuesday(true) if current_weekday_name == 'Tuesday'
      sch_rule.setApplyWednesday(true) if current_weekday_name == 'Wednesday'
      sch_rule.setApplyThursday(true) if current_weekday_name == 'Thursday'
      sch_rule.setApplyFriday(true) if current_weekday_name == 'Friday'
      sch_rule.setApplySaturday(true) if current_weekday_name == 'Saturday'
      sch_rule.setApplySunday(true) if current_weekday_name == 'Sunday'
