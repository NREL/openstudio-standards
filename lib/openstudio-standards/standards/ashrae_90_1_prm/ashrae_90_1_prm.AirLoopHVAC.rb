  ...
  def air_loop_hvac_energy_recovery_ventilator_flow_limit(air_loop_hvac, climate_zone, pct_oa)
    if pct_oa < 0.7
      ...
      air_loop_hvac.thermalZones.each do |thermal_zone|
        ...
        if thermal_zone.additionalProperties.hasFeature('exhaust_energy_recovery_exception_for_deehumidifcation_with_series_cooling_recovery') && thermal_zone.additionalProperties.getFeatureAsBoolean('exhaust_energy_recovery_exception_for_dehumidifcation_with_series_cooling_recovery')
          return nil
        end
      end
