class ASHRAE901PRM2019 < ASHRAE901PRM
  ...
  def model_apply_water_heater_prm_parameter(water_heater_mixed, building_type_swh)
    ...
    #   water_heater_eff = 1
    #   hrly_loss_pct = (0.3 + 27.0/volumn_gal) # unit: %/hr
    #   hrly_loss_value = hrly_loss_pct / 100 # unit: ratio/hr
    #
    #   q = hrly_loss_value * cp * volumn_gal * rho
    #
  ...
  # @return [Boolean] returns true if successful, false if not
  def water_heater_mixed_apply_prm_fuel_type(water_heater_mixed, building_type)
    # Get the fuel type data
