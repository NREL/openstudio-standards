class Standard
  ...
  #
  # @param zones [Array<OpenStudio::Model::ThermalZone>] array of zones to create occupancy schedule
  ...
  def get_zones_occupancy_schedule(model, zones, occupied_percentage_threshold: 0.05)
    ...
    OpenStudio.logFree(OpenStudio::Debug, 'openstudio.model.Model', "The #{zones.size} zone(s) have #{occ_schedules_num_occ.size} unique occ schedules.")
    occ_schedules_num_occ.each do |occ_sch, num_occ|
      OpenStudio.logFree(OpenStudio::Debug, 'openstudio.model.Model', "   #{occ_sch.name} - #{num_occ.round} people")
    end
    OpenStudio.logFree(OpenStudio::Debug, 'openstudio.model.Model', "   Total #{max_occ_in_zones.round} people in #{zones.size} zone(s).")
    ...
    (1..365).each do |i|
      ...
      occ_schedules_num_occ.each do |occ_sch, num_occ|
        ...
        OpenStudio.logFree(OpenStudio::Debug, 'openstudio.model.Model', "Schedule #{occ_sch.name} has #{day_schs.size} day schs") unless day_schs.size == 1
      ...
      # Determine the total fraction for the zones at each time
      daily_times = []
      daily_os_times = []
      daily_values = []
      daily_occs = []
      times_on_this_day.uniq.sort.each do |time|
        ...
        # Total fraction for the zones at each time
        zones_occ_frac = tot_occ_at_time / max_occ_in_zones
        occ_status = 0 # unoccupied
        if zones_occ_frac >= occupied_percentage_threshold
          occ_status = 1
        end
        ...
        daily_occs << zones_occ_frac.round(2)
      end

      # OpenStudio::logFree(OpenStudio::Debug, "openstudio.model.Model", "#{daily_times.join(', ')}                  #{daily_values.join(', ')}")
      ...
      # OpenStudio::logFree(OpenStudio::Debug, "openstudio.model.Model", "#{simple_daily_times.join(', ')}                  {simple_daily_values.join(', ')}")
    ...
    sch_name = "#{zones.size} zone(s) Occ Sch"
    ...
    ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].each do |weekday|
      OpenStudio.logFree(OpenStudio::Debug, 'openstudio.model.Model', weekday.to_s)
      ...
      yearly_data.each_with_index do |daily_data, k|
        ...
        OpenStudio.logFree(OpenStudio::Debug, 'openstudio.model.Model', "Making a new rule for #{weekday} from #{end_of_prev_rule} to #{date}")
        ...
        daily_os_times.each_with_index do |time, t|
          ...
          OpenStudio.logFree(OpenStudio::Debug, 'openstudio.model.Model', "   Adding value #{time}, #{value}")
        end
