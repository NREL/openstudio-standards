    ...
    model.getSpaceTypes.each do |space_type|
      ...
      if (space_type.name.get.downcase.include?('computer')) || (space_type.name.get.downcase.include?('datacenter'))
        if elec_equip_have_info
          ...
          it_equipment_def.setSupplyTemperatureDifference(8.9)   # This is under fully open configuration assumption, based on the lookup table in scorecard
          it_equipment_def.setReturnTemperatureDifference(-7.2)   # This is under fully open configuration assumption, based on the lookup table in scorecard
