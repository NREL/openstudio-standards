...
module LargeDataCenterHighITE
  ...
  def add_data_center_load(model)
    model.getSpaceTypes.each do |space_type|
      ...
      if (space_type.name.get.downcase.include?('computer') || space_type.name.get.downcase.include?('datacenter')) && elec_equip_have_info
        ...
        it_equipment_def.setSupplyTemperatureDifference(9.43) # This is under fully open configuration assumption, based on the lookup table in scorecard
        it_equipment_def.setReturnTemperatureDifference(-7.55) # This is under fully open configuration assumption, based on the lookup table in scorecard
        # after the bug in OpenStudio core is fixed, this temperature schedules was enabled
        it_equipment_def.setSupplyTemperatureDifferenceSchedule(model_add_schedule(model, 'LargeDataCenterHighITE SupplyApproachTemp_SCH'))   # This is under fully open configuration assumption, based on the lookup table in scorecard
        it_equipment_def.setReturnTemperatureDifferenceSchedule(model_add_schedule(model, 'LargeDataCenterHighITE ReturnApproachTemp_SCH'))   # This is under fully open configuration assumption, based on the lookup table in scorecard
  ...
  def get_crah_supply_temp_sch(model)
    ...
    model.getElectricEquipmentITEAirCooledDefinitions.each do |it_equip|
      ## This should be added as a retrofit measure coupled with water-side economizer or free cooling with cooling tower
