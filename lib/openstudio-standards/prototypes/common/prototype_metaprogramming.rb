...
"
  ...
  def initialize
    ...
    @prototype_input = self.model_find_object(table_name: 'prototype_inputs',search_criteria: {'template' => @template,'building_type' => @@building_type })
...
"
  ...
  class #{template}#{name} < #{template}
    ...
    def initialize
      ...
      @template = TEMPLATE
      @instvarbuilding_type = @building_type
      @prototype_input = self.model_find_object(table_name: 'prototype_inputs', search_criteria: {'template' => \"#{template}\",'building_type' => \"#{name}\" })
      if @prototype_input.nil?
...
"
  ...
  def initialize
    super()
    @instvarbuilding_type = @@building_type
    @prototype_input = self.model_find_object(table_name: 'prototype_inputs', search_criteria: {'template' => @template,'building_type' => @@building_type, 'hvac_system' => @@hvac_system})
    if @prototype_input.nil?
