  ...
  module Resources #Resources
    ...
    module Envelope #Resources::Envelope
      ...
      module Constructions #Resources::Envelope::Constructions
        Umin = 0.050 # construction USi upper limit (i.e. R114)
        Umax = 5.678 # construction USi lower limit (i.e. R1)
        ...
        # construction.
        #
        # @author Denis Bourgeois <denis@rd2.ca>
        #
        # @param m [OpenStudio::Model::StandardOpaqueMaterial] a material
        #
        # @return [Boolean] whether material is unique (false if missing)
        def self.material_unique?(m = nil)
          return false unless m.is_a?(OpenStudio::Model::OpaqueMaterial)
        ...
        ##
        # Assigns a unique layered construction material. Solution similar to
        # OpenStudio::Model::LayeredConstruction's 'ensureUniqueLayers', yet
        # limited here to a single indexed OpenStudio material, typically the
        # principal insulating material. Returns true if the indexed material
        # is already unique. Applicable only to standard- or massless-opaque
        # materials.
        #
        # @author Denis Bourgeois <denis@rd2.ca>
        #
        ...
        #
        # @return [Boolean] if successfully assigned (false if invalid inputs)
        def self.assign_unique_material(lc = nil, index = nil)
          return false unless lc.is_a?(OpenStudio::Model::LayeredConstruction)
          return false unless index.is_a?(Integer)
          return false unless index.between?(0, lc.numLayers - 1)

          m = lc.getLayer(index)

          unless m.to_MasslessOpaqueMaterial.empty?
            m = m.to_MasslessOpaqueMaterial.get
            return true if material_unique?(m)
        ...
        # thickness. If a thickness-adjusted insulating layer already exists,
        # then this is recovered instead of instantiating a new one. The latter
        # is renamed according to its adjusted thickness (or RSi value).
        #
        # @author Denis Bourgeois <denis@rd2.ca>
        #
        # @param lc [OpenStudio::Model::LayeredConstruction] a construction
        ...
        #
        # @return [Float] insulating layer RSi (0.0 if invalid input)
        def self.reset_uo(lc = nil, film = nil, index = nil, uo = nil)
          return 0 unless film.is_a?(Numeric)
          return 0     if film.negative?
          return 0 unless uo.is_a?(Numeric)
          return 0 unless uo.between?(Umin, Umax)
          return 0 unless lc.is_a?(OpenStudio::Model::LayeredConstruction)
          return 0 unless index.is_a?(Integer)
          return 0 unless index.between?(0, lc.numLayers - 1)

          r   = 0                 # thermal resistance of material
          r0  = TBD.rsi(lc, film) # current construction RSi value
          ...
          unless m.to_MasslessOpaqueMaterial.empty?
            ...
            return r if (r + dR) < 0.001

            r += dR
            id = "BTAP:#{format('%.2f', r)}RSi"
            ...
            unless mt.empty?
              ...
              if r.round(2) == mt.thermalResistance.round(2)
          ...
          unless m.to_StandardOpaqueMaterial.empty?
            ...
            d = (r + dR) * m.conductivity
            return r if d < 0.03

            r  = d / m.conductivity
            id = "BTAP:#{format('%03d', d*1000)[-3..-1]}mm"
            ...
            unless mt.empty?
              ...
              if r.round(2) == (mt.thickness / mt.conductivity).round(2)
                lc.setLayer(index, mt)
                return r
              end
            end
