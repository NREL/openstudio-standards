...
class BTAPResults_Test < Minitest::Test
  ...
  def create_model_simulate_and_qaqc_regression_test(epw_file:,
    ...
    if @test_file.nil?
      ...
      model_name = "#{building_type}-#{template}-DefaultFuel-#{File.basename(epw_file, '.epw')}_SHW"  #NOTE: "primary_heating_fuel" has been set to "DefaultFuel" instead of 'FuelOilNo2' and its associated "model_name" has been changed in a way to use an expected result file that already exits on github.
      puts model_name
